...
## Current Milestone: v0.2 智能调研与文档生成（合并版）

**Goal:** 实现调研辅助（实时问题推荐、录音转写、记录）和智能文档生成（模板解析、LLM 填充、8 阶段交付物自动生成），打通调研→报告闭环。

**Target features:**
- 调研提纲知识库：解析 25 份企业版调研提纲，Edition Profile 框架（企业版/旗舰版）
- 问题推荐引擎：实时语义匹配，根据客户对话推荐相关问题
- Whisper 本地语音识别：离线免费，实时转写
- 调研记录：调研过程全记录，Q&A 追踪
- 模板解析引擎：解析 85 个交付物模板的字段、章节结构
- 文档生成核心：LLM 填充模板，输出标准化 .docx/.xlsx
- 智能补全：从知识库检索相关历史内容辅助填充
- 产品管理：产物浏览、导出、历史记录
- 统一前端：调研提示板 + 向导式文档生成 + 腾讯会议侧边栏
