...
| 代码正确性 | ⭐⭐⭐ | 3 处阻断/高优先级错误（见下文 10.1-10.3） |
| 依赖完整性 | ⭐⭐⭐ | 2 处遗漏依赖和文件引用 |
...
└── risk_control.rs [改]    ← ❌ 文件不存在
```

**实际文件**：
- `src-tauri/src/commands/risk_blueprint.rs` — 含 `agent_chat`(L310)、`cancel_agent_stream`(L448)、`answer_question`(L433)
- `src-tauri/src/services/risk_control.rs` — 风险管理服务层，非命令层

**影响**：P0-e 实施时找不到 `risk_control.rs` 命令文件，编译失败。

**修复**：计划 L49 以及所有后续引用（涉及渐进式披露接入修改）中，`risk_control.rs` **应改为 `risk_blueprint.rs`**。
同时，以下文档的引用也需同步修正：
- `ARCHITECTURE.md` L273（序列图）→ 改为 `risk_blueprint.rs`
- 设计文档中全部 `risk_control.rs` 引用 → 改回 `risk_blueprint.rs`（之前错误地 "修正" 成了错误文件名）
...
### 🔴 10.3 缺失 `search_llm.rs` 中 `count_tokens` Tauri 命令的迁移（阻断）

**位置**：计划 Task 1 Step 3（L221-226）
**问题**：计划将 `llm_service.rs` 中的 `count_tokens` 和 `truncate_to_tokens` 迁移到 `token.rs`，但仅提及了 `llm_service.rs` 内的调用点（L502, L1334, L1369, L1947）。

**遗漏**：`commands/search_llm.rs:113` 有一个 **Tauri 命令**封装了 `llm_service::count_tokens`：

```rust
// search_llm.rs:111-114
#[tauri::command]
...
**修复**：在 Task 1 Step 3 中增加：
> 修改 `commands/search_llm.rs:113`，将 `llm_service::count_tokens` 改为 `token::count_tokens`

---

## 11. 高风险问题

### 🟡 11.1 `bitflags` 依赖添加时序错误

**位置**：Task 4（L557-595）vs Task 5（L766）
**问题**：
- Task 4：创建 `types.rs`，使用 `bitflags!` 宏（`use bitflags::bitflags;`）
- Task 5：执行 `cargo add bitflags`（L766）

`bitflags` 必须在 **Task 4** 编译前添加，而不是 Task 5。当前时序会导致 Task 4 编译失败。

**修复**：将 `cargo add bitflags` 移到 Task 4，或在 Task 4 中注明先添加依赖。
...
|---|---------|---------|---------|---------|--------|
| Tauri 命令文件 | `risk_control.rs` ❌ | L51: `risk_blueprint.rs` ✅ / L273: `risk_control.rs` ❌ | `risk_control.rs` ❌ | `risk_blueprint.rs` ✅ | ❌ |
...
| # | 问题 | 优先级 | 涉及文档 | 说明 |
|---|------|--------|---------|------|
| 1 | 命令文件名错乱（`risk_control.rs` ↔ `risk_blueprint.rs`） | 🔴 阻断 | 设计文档、架构文档、计划 | 三文档全部需要修正 |
| 2 | 计划 L32-L36 重复条目 | 🔴 阻断 | 计划 | 删除重复行 |
| 3 | 计划缺失 `search_llm.rs` 迁移 | 🔴 阻断 | 计划 | Task 1 Step 3 补充 |
| 4 | `bitflags` 依赖时序 | 🟡 高 | 计划 | Task 4 添加依赖，非 Task 5 |
| 5 | Task 1 Step 0 与 Task 2 重叠 | 🟡 高 | 计划 | 合并或删除冗余 |
| 6 | `uuid` / `tiktoken-rs` 冗余添加 | 🟡 高 | 计划 | 删除 `cargo add uuid`（已存在） |
| 7 | `maybe_reset` 未用 `reset_threshold_pct` | 🟢 低 | 设计文档 | L459-464 改 `release_target_pct` → `reset_threshold_pct` |
| 8 | U1: Speculative Execution 并发 | 🟢 低 | 设计文档、架构文档 | P1-b 实施时处理 |
| 9 | U2: 字符硬上限安全阀 | 🟢 低 | 设计文档、架构文档、计划 | 实施时添加 |
| 10 | `"summarization"` 模型标签映射未定义 | 🟢 低 | 设计文档、计划 | 需在配置中增加 `summarization_model` 字段 |
| 11 | 架构文档 ChatMessage 未更新 | 🟢 低 | 架构文档 | 更新结构体 |
| 12 | 计划中 line number 漂移风险 | 🟢 低 | 计划 | 标注函数名作为备用 |
| 13 | `git add -A` 过于宽泛 | 🟢 低 | 计划 | 改为显式路径 |
...
| 任务 | 前置条件 | 风险 |
|------|---------|------|
| Task 4: `types.rs` + `bitflags` | 添加 `bitflags` 依赖 | 无 |
| Task 3: `model_metadata.rs` + `model_specs.json` | 无（纯新增） | 低 |
| Task 1: `token.rs`（不含 Step 0） | 无（纯新增） | 低 |
...
| Task 1 Step 3: 删除 `llm_service` 旧函数 | 缺失 `search_llm.rs` 迁移步 | 补充迁移步 |
| Task 1 Step 3: 替换调用点 | line number 漂移 | 确认实际 line number |
| P0-e: 渐进式披露 | `risk_control.rs` 不存在 | 改为 `risk_blueprint.rs` |
| 架构文档更新 | 多处过时 | 批量修正 |
...
| 第 4 轮（本轮） | 架构文档 + 实现计划 | 13 项（3 阻断、3 高、7 低） | 待修复 |

**结束** — 文档可进入实施阶段（需先修复 3 个阻断缺陷）。
