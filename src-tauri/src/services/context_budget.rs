...
impl ContextBudget {
    pub fn calculate(metadata: &ModelMetadata, mode: AgentMode) -> Self {
        ...
        if total_ideal > 0 {
            for claim in &claims {
                ...
                let share = if remaining > 0 { remaining * deficit / total_ideal } else { 0 };
    ...
    fn build_claims(total: u32, reserved_output: u32, mode: AgentMode) -> Vec<BudgetClaim> {
        ...
        vec![
            BudgetClaim { slot: BudgetPriority::SystemPrompt,   min_tokens: 200,             ideal_tokens: total / 10,       mode_mask: AgentMode::all() },
            BudgetClaim { slot: BudgetPriority::UserInput,      min_tokens: 100,             ideal_tokens: total / 20,       mode_mask: AgentMode::all() },
            BudgetClaim { slot: BudgetPriority::ReservedOutput, min_tokens: reserved_output, ideal_tokens: reserved_output,  mode_mask: AgentMode::all() },
            BudgetClaim { slot: BudgetPriority::ToolDefs,       min_tokens: 0,               ideal_tokens: total / 5,        mode_mask: if has_tools { AgentMode::all() } else { AgentMode::empty() } },
            BudgetClaim { slot: BudgetPriority::Plan,           min_tokens: 0,               ideal_tokens: total / 4,        mode_mask: if has_plan { AgentMode::all() } else { AgentMode::empty() } },
            BudgetClaim { slot: BudgetPriority::History,        min_tokens: 500,             ideal_tokens: total / 2,        mode_mask: AgentMode::all() },
            BudgetClaim { slot: BudgetPriority::RetrievedCtx,   min_tokens: 0,               ideal_tokens: total * 3 / 10,   mode_mask: AgentMode::all() },
            BudgetClaim { slot: BudgetPriority::Buffer,         min_tokens: 200,             ideal_tokens: 500,              mode_mask: AgentMode::all() },
        ]
    }
}
...
mod tests {
    ...
    fn test_budget_calculate_react_mode() {
        let meta = ModelMetadata {
            ...
            supports_thinking: false, supports_vision: false, supports_tools: true,
        };
    ...
    fn test_budget_not_exceed_total() {
        let meta = ModelMetadata {
            ...
            supports_thinking: false, supports_vision: false, supports_tools: false,
        };
